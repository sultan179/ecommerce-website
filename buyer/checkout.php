...
<body>
  ...
  <!-- <div class="container-fluid">
        <div class="row">
            <div class="col-sm-4 text-left">
                <button href="myCart.html" class="btn btn-primary" type="submit">My cart: 1 item</button>
            </div>
  ...
  <div class="container">
    <div class="row">
      <!-- <div class=""> -->
      <h2 class="text-center mt-3 mb-3">order info</h2>
      <!-- </div> -->
  ...
  include('../dbconfig.php');
    if (isset($_POST['checkout-btn'])) {
            ...
            $cardnumber = $POST['cnumber'];
            $edate = $POST['edate'];
            $cvv = $POST['cvv'];
            $order_id = uniqid();

            $sql ="SELECT product.seller from product
            inner join cart
            on cart.product_id=product.id
            where cart.id={$firstCart}";
            $result = mysqli_query($conn,$sql);
            $row=mysqli_fetch_row($result);
            $seller = $row[0];
            $sql = "INSERT into orders(order_id,order_status,cart_items,buyer,seller) VALUES('$order_id','In Progress','$cart_items','$user_email','{$seller}')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                $sql = "INSERT into order_details(fname,lname,phone,email,b_address,s_address,country,state,zip,order_id) values('${firstname}','${lastname}','${phone}','${email}','${baddress}','${saddress}','${country}','${state}','${zip}','${order_id}')";
                if ($result = mysqli_query($conn, $sql)) {
                  $sql = "INSERT into payment_information(card_owner,card_number,cvv,expiry_date,order_id) values('${cardname}','${cardnumber}','${cvv}','${edate}','${order_id}')";
                  if($result = mysqli_query($conn, $sql)){
                    $array_details = array($order_id,$email,$baddress,$zip,$phone,$grand_total);
                    $_SESSION['summary'] = $array_details;
                    echo "<script>window.location.replace('order-summary.php')</script>";
                    // header('Location: ./user/plans.php');
                  }else{
                    echo "<p class='text-center alert alert-danger mt-4 mb-4'>Error Checking out Inner</p>";
                  }     
                } else {
                    $sql = "DELETE from orders where order_id='${order_id}'";
                    mysqli_query($conn,$sql);
                    echo "<p class='text-center alert alert-danger mt-4 mb-4'>Error Checking out Outer</p>";
                }
            }else{
            echo "<p class='text-center alert alert-danger mt-4 mb-4'>Error Checking out Outer most</p>";
            }
    }
    ?>
      <hr>
      ...
      <label>
        ...
        <input type="text" name="cvv" required /> </label>
      <button type="submit" class="registerbtn" name="checkout-btn">Checkout</button>
  </form>
</body>
